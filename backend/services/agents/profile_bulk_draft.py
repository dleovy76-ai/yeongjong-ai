...
정리).
- opening_hours(영업시간)가 요일별로 다르면 "요일 시작-종료" 형식으로 요일마다 세미콜론(;)으로 \
구분해서 정리하세요. 예: "월-토 11:00-21:00; 일 휴무". 브레이크타임(휴게시간)은 여기 섞지 말고 \
반드시 별도의 break_time 항목에 적으세요. 같은 단어를 문장 끝에 의미 없이 중복해서 쓰지 마세요.
- break_time(브레이크타임)은 영업시간과 분리된 별도 항목입니다 - 브레이크타임이 안 보이면 null로 \
두세요.
- 리뷰, 별점, 광고 문구처럼 아래 항목에 해당하지 않는 내용은 무시하세요.
- 다른 설명 없이 아래 형식의 JSON 객체 하나만 응답하세요(값을 모르면 null):
{"description": "...", "opening_hours": "...", "break_time": "...", "holiday": "...", \
"parking": "...", "pet_policy": "...", "reservation_policy": "...", "takeout_policy": "...", \
"payment_methods": "..."}"""


class ProfileBulkDraftAgent(BaseAgent):
    """사장님이 네이버 플레이스 등에서 직접 캡쳐해 올린 이미지를 보고 업체
    프로필 항목(가게소개/영업시간/브레이크타임/휴무일/주차/반려동물/예약안내/
    포장안내/결제수단) 후보를 뽑아주는 도우미. 자동 스크래핑이 아니라 사장님이 직접
