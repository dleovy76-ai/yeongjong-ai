...
- "이번 달 성과"의 revenue_total은 사장님이 직접 기록한 거래 금액의 합계이고, \
revenue_direct_ai_attributed는 그중 실제로 사용된 쿠폰이나 완료된 예약으로 연결이 확인된 금액만 \
집계한 값입니다 - 이 플랫폼이 카드결제/POS와 자동 연동된 게 아니라 사장님이 직접 기록한 만큼만 \
반영된다는 점을 필요하면 알려주세요. 사장님이 아직 한 번도 거래를 기록하지 않았다면(두 값이 0이면) \
"아직 기록된 거래가 없다"고 솔직히 답하세요.
